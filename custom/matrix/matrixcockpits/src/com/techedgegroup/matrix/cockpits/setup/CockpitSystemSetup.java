 ...
 */
package com.techedgegroup.matrix.cockpits.setup;
...
import com.techedgegroup.matrix.cockpits.constants.MatrixCockpitsConstants;
 ...
 */
@SystemSetup(extension = MatrixCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(MatrixCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(MatrixCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						MatrixCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/matrixcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
